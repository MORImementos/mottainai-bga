...
function(dojo, declare) {
	return declare('bgagame.mottainaicardgame', ebg.core.gamegui, {
		...
		*/
		...
		onEnteringState: function(stateName, args) {
			...
			case 'client_doClerk':
				...
				dojo.query(`#player_${player_id}_craft_bench > .card_image`).forEach((node, index, arr) => {
					this.selectableElements.push(node);
					node.classList.add('selectable');
		...
		*/
		...
		onSelectingCardToSmith: function(event) {
			...
			if (support_required) {
				...
				let galleryNodes = dojo.query(`#player_${player_id}_gallery`);
		...
		onChoosingWing: function(event) {
			console.log('onchoosingwing is running');
			dojo.stopEvent(event);
			if (this.isInterfaceLocked()) return;
			if (!this.checkAction('chooseAction')) return;

			let wingElement = event.currentTarget;
			let wingId = wingElement.id;

			let wing = '';
			if (wingId.includes('gallery')) {
				wing = 'gallery';
			} else if (wingId.includes('gift_shop')) {
				wing = 'gift_shop';
			}

			if (wing) {
				this.doSmithCraftWing(event, wing);
			}
		},
